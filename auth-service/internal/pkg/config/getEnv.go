...
func Load() Config {
	...
	config.AUTH_GRPC_PORT = cast.ToString(coalesce("AUTH_GRPC_PORT", ":50050"))
	config.AUTH_HTTP_PORT = cast.ToString(coalesce("AUTH_HTTP_PORT", ":8088"))
	...
	config.DB_NAME = cast.ToString(coalesce("DB_NAME", "muallimah_db"))
